...
class NowPlayingDataSource(
    ...
    override fun loadInitial(
        ...
        Log.d(TAG, " >>> Received call to loadInitial for nowPlaying movies -> Request Type: $requestType")
        state = state.copy(initialLoading = true, success = false, failure = false, message = null, eventType = requestType)
        compositeDisposable.add(
            moviesRepoI.getNowPlaying(lastRequestedPage++)
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, Result>) {
        Log.d(TAG, " >>> Received call to loadAfter for nowPlaying movies")
    ...
    fun retry() {
        Log.d(TAG, " >>> Received call to retry now playing request")
        val prevRetry = retry
        retry = null
        lastRequestedPage = 1
        prevRetry?.invoke()
    ...
    companion object {
        const val TAG = "NowPlayingDataSource"
    }
