...
class UpcomingDataSource(
    ...
    override fun loadInitial(
        ...
        Log.d(TAG, " >>> Received call to loadInitial for upcoming movies -> Request Type: $requestType")
        state = state.copy(initialLoading = true, success = false, failure = false, message = null, eventType = requestType)
        compositeDisposable.add(
            moviesRepoI.getUpcomingMovies(lastRequestedPage++)
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, Result>) {
        Log.d(TAG, " >>> Received call to loadAfter for upcoming movies")
    ...
    companion object {
        const val TAG = "UpcomingDataSource"
    }
}